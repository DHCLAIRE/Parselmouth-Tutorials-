...
- Praat source: `SyllableNucleiv3.praat`
- Tutorial type: `process`
- Selection model: one or more selected `Sound` objects

## What The Praat Script Does

- Runs the logic in `SyllableNucleiv3.praat` from the Vocal Toolkit plugin.
- Pitch analysis.
...
SCRIPT = os.path.join(TOOLKIT_DIR, "SyllableNucleiv3.praat")
...
result = run_toolkit_script("SyllableNucleiv3.praat", "voice.wav")
```
...
- This script calls or includes: `FilledPauses.praat`, `also`, `number`.
