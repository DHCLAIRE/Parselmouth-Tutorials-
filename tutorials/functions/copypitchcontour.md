...
- Praat source: `copypitchcontour.praat`
- Tutorial type: `copy`
- Selection model: two selected `Sound` objects

## What The Praat Script Does

- Runs the logic in `copypitchcontour.praat` from the Vocal Toolkit plugin.
- Pitch analysis.
...
SCRIPT = os.path.join(TOOLKIT_DIR, "copypitchcontour.praat")
...
result = run_toolkit_script("copypitchcontour.praat", ["source.wav", "target.wav"])
```
...
- This script calls or includes: `minmaxf0.praat`, `workpost.praat`, `workpre.praat`.
