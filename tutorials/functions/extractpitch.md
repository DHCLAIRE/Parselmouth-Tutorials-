...
- Praat source: `extractpitch.praat`
- Tutorial type: `process`
- Selection model: one or more selected `Sound` objects

## What The Praat Script Does

- Runs the logic in `extractpitch.praat` from the Vocal Toolkit plugin.
- Pitch analysis.
...
SCRIPT = os.path.join(TOOLKIT_DIR, "extractpitch.praat")
...
result = run_toolkit_script("extractpitch.praat", "voice.wav")
```
...
- This script calls or includes: `batch.praat`, `minmaxf0.praat`.
