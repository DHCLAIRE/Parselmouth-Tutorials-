...
- Praat source: `voicedunvoiced.praat`
- Tutorial type: `process`
- Selection model: one or more selected `Sound` objects

## What The Praat Script Does

- Runs the logic in `voicedunvoiced.praat` from the Vocal Toolkit plugin.
...
| --- | --- | --- | --- |
| `Create TextGrid` | `boolean` | `0` | `create_textgrid` |
...
SCRIPT = os.path.join(TOOLKIT_DIR, "voicedunvoiced.praat")

sound = parselmouth.Sound("voice.wav")
result = praat.run_file(sound, SCRIPT, 0)
...
- This script calls or includes: `batch.praat`, `fixdc.praat`, `minmaxf0.praat`, `workpost.praat`, `workpre.praat`.
