...
- Praat source: `copymix.praat`
- Tutorial type: `copy`
- Selection model: two selected `Sound` objects

## What The Praat Script Does

- Runs the logic in `copymix.praat` from the Vocal Toolkit plugin.
...
| --- | --- | --- | --- |
| `Mix (%)` | `real` | `50` | `mix` |
| `Avoid clipping` | `boolean` | `0` | `avoid_clipping` |
...
SCRIPT = os.path.join(TOOLKIT_DIR, "copymix.praat")

source = parselmouth.Sound("source.wav")
target = parselmouth.Sound("target.wav")
result = praat.run_file([source, target], SCRIPT, 50, 0, 1)
...
- This script calls or includes: `declip.praat`, `fixdc.praat`, `preview.inc`.
