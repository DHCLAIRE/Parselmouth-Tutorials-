# Normalize (scale peak 0.99)

- Praat source: `normalize.praat`
- Tutorial type: `process`
- Selection model: one or more selected `Sound` objects

## What The Praat Script Does

- Runs the logic in `normalize.praat` from the Vocal Toolkit plugin.
...
SCRIPT = os.path.join(TOOLKIT_DIR, "normalize.praat")
...
result = run_toolkit_script("normalize.praat", "voice.wav")
```
