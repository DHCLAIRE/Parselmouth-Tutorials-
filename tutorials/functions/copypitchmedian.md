...
- Praat source: `copypitchmedian.praat`
- Tutorial type: `copy`
- Selection model: two selected `Sound` objects

## What The Praat Script Does

- Runs the logic in `copypitchmedian.praat` from the Vocal Toolkit plugin.
- Pitch analysis.
...
SCRIPT = os.path.join(TOOLKIT_DIR, "copypitchmedian.praat")

source = parselmouth.Sound("source.wav")
target = parselmouth.Sound("target.wav")
result = praat.run_file([source, target], SCRIPT, 1, 1, 1, 1)
...
- This script calls or includes: `minmaxf0.praat`, `preview.inc`, `workpost.praat`, `workpre.praat`.
