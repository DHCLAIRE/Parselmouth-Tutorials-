...
- Praat source: `copyintensitycontour.praat`
- Tutorial type: `copy`
- Selection model: two selected `Sound` objects

## What The Praat Script Does

- Runs the logic in `copyintensitycontour.praat` from the Vocal Toolkit plugin.
...
SCRIPT = os.path.join(TOOLKIT_DIR, "copyintensitycontour.praat")
...
result = run_toolkit_script("copyintensitycontour.praat", ["source.wav", "target.wav"])
```
...
- This script calls or includes: `declip.praat`, `workpost.praat`, `workpre.praat`.
