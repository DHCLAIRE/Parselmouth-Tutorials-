...
- Praat source: `copyeq.praat`
- Tutorial type: `copy`
- Selection model: two selected `Sound` objects

## What The Praat Script Does

- Runs the logic in `copyeq.praat` from the Vocal Toolkit plugin.
...
SCRIPT = os.path.join(TOOLKIT_DIR, "copyeq.praat")
...
result = run_toolkit_script("copyeq.praat", ["source.wav", "target.wav"])
```
...
- This script calls or includes: `declip.praat`, `ireq.praat`, `workpost.praat`, `workpre.praat`.
