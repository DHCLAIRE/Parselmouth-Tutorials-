...
- Praat source: `copyvocoder.praat`
- Tutorial type: `copy`
- Selection model: two selected `Sound` objects

## What The Praat Script Does

- Runs the logic in `copyvocoder.praat` from the Vocal Toolkit plugin.
...
SCRIPT = os.path.join(TOOLKIT_DIR, "copyvocoder.praat")

source = parselmouth.Sound("source.wav")
target = parselmouth.Sound("target.wav")
result = praat.run_file([source, target], SCRIPT)
...
result = run_toolkit_script("copyvocoder.praat", ["source.wav", "target.wav"])
```
...
- This script calls or includes: `copyintensitycontour.praat`, `declip.praat`, `gate.praat`.
