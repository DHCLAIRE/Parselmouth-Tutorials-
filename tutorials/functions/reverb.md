...
- Praat source: `reverb.praat`
- Tutorial type: `process`
- Selection model: one or more selected `Sound` objects

## What The Praat Script Does

- The following list is updated when Praat starts up with the reverb
- impulse response files found in a subfolder called “reverb”.
...
| --- | --- | --- | --- |
| `Preset` | `optionmenu` | `1; options: Ambience, Bath, Church, Hall, Plate, Robot, Room Big, Room Medium, ...` | `preset` |
| `Mix (%)` | `real` | `50` | `mix` |
...
SCRIPT = os.path.join(TOOLKIT_DIR, "reverb.praat")

sound = parselmouth.Sound("voice.wav")
result = praat.run_file(sound, SCRIPT, 1, 50, 1)
...
- Reverb presets rely on impulse-response files in the toolkit `reverb/` folder. Keep the plugin folder intact so relative `Read from file` calls can resolve the WAV files.
...
- This script calls or includes: `batch.praat`, `copymix.praat`, `fixdc.praat`, `preview1.inc`, `preview2.inc`, `reverbpresetslist.inc`.
