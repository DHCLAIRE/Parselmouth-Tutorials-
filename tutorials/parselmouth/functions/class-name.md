# class_name

- Kind: `property`
- Available on: `CC`, `Data`, `Formant`, `Function`, `Harmonicity`, `Intensity`, `MFCC`, `Matrix`, `Pitch`, `Sampled`, `SampledXY`, `Sound`, `Spectrogram`, `Spectrum`, `TextGrid`, `Thing`, `TimeFrameSampled`, `TimeFunction`, `Vector`
- Vocal Toolkit coverage: **Related**

## What It Does

This public Parselmouth API member is documented in the official API reference.

## Tutorial Pattern

```python
# Replace `obj` with an instance of CC.
value = obj.class_name
print(value)
```

## Signature

```text
See the official API reference for overload details.
```

## Toolkit Comparison

A related term appears in the Vocal Toolkit command set or scripts.

[Back to Parselmouth API index](../index.md)
